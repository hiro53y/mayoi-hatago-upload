...
export function runGameFlowTests(): void {
  ...
  greaterThanOrEqual(99, state.player.hunger, 'hunger should not increase after movement');

  const wallState = JSON.parse(JSON.stringify(state)) as GameState;
  wallState.player.position = { x: 0, y: 0 };
  const blocked = performPlayerAction(wallState, { type: 'move', dx: -1, dy: 0 });
